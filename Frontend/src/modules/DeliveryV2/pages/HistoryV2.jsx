...
import { 
  ...
  CheckCircle2, Clock, Search, History, Star
} from 'lucide-react';
          ...
          {loading ? (
             ...
             <div className="space-y-4">
                {trips.map((trip, idx) => {
                   ...
                   const isCOD = (trip.paymentMethod || '').toLowerCase() === 'cash' || (trip.paymentMethod || '').toLowerCase() === 'cod';
                   const customerRating = Number(trip.deliveryPartnerRating?.rating || 0);
                   ...
                   return (
                      <div key={trip.orderId || idx} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm active:scale-[0.99] transition-all">
                         ...
                         {isCompleted && customerRating > 0 && (
                            <div className="flex items-center gap-2 mb-4 rounded-xl bg-amber-50 border border-amber-100 px-3 py-2">
                               <span className="text-[11px] font-semibold text-amber-700">Customer rating</span>
                               <div className="flex items-center gap-0.5" aria-label={`${customerRating} out of 5 stars`}>
                                  {[1, 2, 3, 4, 5].map((star) => (
                                     <Star
                                        key={star}
                                        className={`w-3.5 h-3.5 ${star <= customerRating ? 'fill-amber-400 text-amber-400' : 'text-amber-200'}`}
                                     />
                                  ))}
                               </div>
                               <span className="text-xs font-bold text-amber-800">{customerRating}/5</span>
                            </div>
                         )}
