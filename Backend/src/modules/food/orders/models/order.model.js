...
const subscriptionUsageSchema = new mongoose.Schema(
    {
        ...
        allocatedDeliveryFeeAmount: { type: Number, default: 0, min: 0 },
        allocatedPlatformFeeAmount: { type: Number, default: 0, min: 0 },
        allocatedGstAmount: { type: Number, default: 0, min: 0 },
        allocatedCouponDiscountAmount: { type: Number, default: 0, min: 0 },
        totalAllocatedRevenueAmount: { type: Number, default: 0, min: 0 },
...
orderSchema.virtual('pricing')
    .get(function () {
        const fallbackTotal =
            Number(this.subscriptionUsage?.operationalOrderValue || 0) ||
            Number(this.totalAmount || 0) ||
            Number(this.subtotal || 0);
        const fallbackSubtotal =
            Number(this.subtotal || 0) ||
            fallbackTotal;
        const subscriptionCreditApplied =
            Number(this.subscriptionUsage?.subscriptionCreditApplied || 0);
        return {
            ...
            tcs: 0,
            discount: 0,
            originalTotal: fallbackSubtotal,
            payableTotal: 0,
